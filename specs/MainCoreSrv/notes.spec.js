...
describe("Notes.spec tests | ", function() {
  ...
  function assertsFx(error, body) {
    ...
    if (body.Value.length > 0) {
      expect(body.Value[0].NoteID).not.toBeNull("NoteID should never be null.");
      expect(body.Value[0].NoteTypeId).not.toBeNull("NoteTypeId should never be null.");
      expect(body.Value[0].CustomerMasterFileId).not.toBeNull("CustomerMasterFileId should never be null.");
      expect(body.Value[0].CustomerId).toBeDefined("CustomerId could be null but should be defined.");
      expect(body.Value[0].LeadId).toBeDefined("LeadId could be null but should be defined.");
      expect(body.Value[0].NoteCategory1Id).not.toBeNull("NoteCategory1Id can not null.");
      expect(body.Value[0].NoteCategory2Id).toBeDefined("NoteCategory2Id could be null but should be defined.");
      expect(body.Value[0].Note).not.toBeNull("Note should not be null.");
    }
  }
