/** EmergencyContactPhoneTypes.spec.js */
...
describe("EmergencyContactPhoneTypes.spec tests | ", function() {
  // ** START BEFORE EACH.
  ...
  it("Get all EMC phone types", function(done) {
    request.get({
      url: config.SseServicesCmsCORS + "MsAccountSetupSrv/EmergencyContactPhoneTypes",
      json: true
      ...
      body.Value.map(function(item) {
        modelDef.msEmergencyContactPhoneType(item);
      });
